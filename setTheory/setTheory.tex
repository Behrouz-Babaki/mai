...
Which of the following statements are always true (for all sets)? 
% which are always false (AF), and which may be true or false depending on the sets (TF) ?
\begin{solution}
\begin{multiple-choice}
\choice[correct] $(C \setminus A) \setminus B = C \setminus (A \cup B)$
\choice[correct] $C \setminus (A \cup B) = (C \setminus A) \cap (C\setminus B)$
\end{multiple-choice}
\end{solution}
\end{question}
